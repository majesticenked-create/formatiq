...
const SAMPLE = '.card{padding:16px;border-radius:8px;background:#fff}.card__title{font-size:18px;font-weight:600}';

function tryFormat(input: string, indent: number) {
  if (!input.trim()) {
    return { ok: false as const, message: 'Paste some CSS to format.' };
  }
  ...
  try {
    ...
    return { ok: false as const, message: err instanceof Error ? err.message : 'Could not format this CSS.' };
  }
}
