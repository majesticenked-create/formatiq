...
const SAMPLE = '<!DOCTYPE html><html><head><title>Formatiq</title></head><body><h1>Hello, world!</h1><p>Sample paragraph.</p></body></html>';

function tryFormat(input: string, indent: number) {
  if (!input.trim()) {
    return { ok: false as const, message: 'Paste some HTML to format.' };
  }

  try {
    const output = html_beautify(input, { indent_size: indent });
    return { ok: true as const, output };
  } catch (err) {
    return { ok: false as const, message: err instanceof Error ? err.message : 'Could not format this HTML.' };
  }
}
