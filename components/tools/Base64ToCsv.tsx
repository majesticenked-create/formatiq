...
const SAMPLE = 'aWQsbmFtZSxub3RlcwoxLEZvcm1hdGlxLCJGcmVlLCBicm93c2VyLWJhc2VkIHRvb2xzIg==';

type Stage = 'base64' | 'utf8' | 'csv';
...
function convert(input: string) {
  ...
  if (!trimmed) {
    return { ok: false as const, stage: 'csv' as Stage, message: 'Decoded text is empty - no rows to parse.' };
  }

  const rows = parseDelimitedRows(trimmed, ',').filter((r) => r.length > 1 || r[0] !== '');
  if (rows.length < 1) {
    return { ok: false as const, stage: 'csv' as Stage, message: 'No CSV rows found in the decoded text.' };
  }
  ...
  return (
    <div>
      ...
      <div className="status-line status-neutral" style={{ marginTop: 12 }}>
        ...
        bytes form valid UTF-8 text, (3) parsing that text as CSV with a quote/escape-aware parser -
        the same parser used by <a href="/tools/converters/csv-tsv-converter">CSV ⇄ TSV Converter</a>
        - so fields containing commas or newlines inside quotes are preserved correctly rather than
        naively split on every comma. Runs entirely client-side.
      </div>
    </div>
  );
}
