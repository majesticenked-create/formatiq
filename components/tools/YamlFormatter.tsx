...
function tryFormat(input: string, indent: number) {
  if (!input.trim()) {
    return { ok: false as const, message: 'Paste some YAML to format.' };
  }
  ...
  try {
    const output = dump(parsed, { indent });
    return { ok: true as const, output };
  } catch (err) {
    return { ok: false as const, message: err instanceof Error ? err.message : 'Could not re-format this YAML.' };
  }
}
